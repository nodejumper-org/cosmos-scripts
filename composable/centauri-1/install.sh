...
BINARY_VERSION_TAG="v5.2.2-client-fix"
...
curl -s https://snapshots.nodejumper.io/composable/addrbook.json > $HOME/.banksy/config/addrbook.json
...
SNAP_NAME=$(curl -s https://snapshots.nodejumper.io/composable/info.json | jq -r .fileName)
curl "https://snapshots.nodejumper.io/composable/${SNAP_NAME}" | lz4 -dc - | tar -xf - -C "$HOME/.banksy"
