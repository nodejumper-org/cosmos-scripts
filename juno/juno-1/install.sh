...
sed -i 's|^seeds *=.*|seeds = "'$SEEDS'"|; s|^persistent_peers *=.*|persistent_peers = "'$peers'"|' $HOME/.juno/config/config.toml
...
sed -i 's|pruning-interval = "0"|pruning-interval = "17"|g' $HOME/.juno/config/app.toml
