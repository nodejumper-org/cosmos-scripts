...
make install
ununifid version # v1.0.0-beta.4
...
# TODO curl -s https://snapshots.nodejumper.io/ununifi/addrbook.json > $HOME/.ununifi/config/addrbook.json
...
sed -i 's|^snapshot-interval *=.*|snapshot-interval = 2000|g' $HOME/.ununifi/config/app.toml
...
# TODO:
#SNAP_NAME=$(curl -s https://snapshots-testnet.nodejumper.io/ununifi/info.json | jq -r .fileName)
#curl "https://snapshots.nodejumper.io/ununifi/${SNAP_NAME}" | lz4 -dc - | tar -xf - -C $HOME/.ununifi
