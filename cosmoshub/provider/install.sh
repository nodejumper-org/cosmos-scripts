...
curl -s https://snapshots-testnet.nodejumper.io/cosmos-testnet/addrbook.json > $HOME/.gaia/config/addrbook.json
...
SNAP_NAME=$(curl -s https://snapshots-testnet.nodejumper.io/cosmos-testnet/info.json | jq -r .fileName)
curl "https://snapshots-testnet.nodejumper.io/cosmos-testnet/${SNAP_NAME}" | lz4 -dc - | tar -xf - -C "$HOME/.gaia"
