...
sed -i 's|^pruning-interval *=.*|pruning-interval = "10"|g' $HOME/.starnamed/config/app.toml
sed -i 's|^snapshot-interval *=.*|snapshot-interval = 0|g' $HOME/.starnamed/config/app.toml
